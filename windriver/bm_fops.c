...
//
    ...
    case BMDEV_FREE_GMEM_U64:
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_DEVICE, "Start [BMDEV_FREE_GMEM] , command code is %d\n", IoControlCode);
        ret = bmdrv_gmem_ioctl_free_mem_u64(
            bmdi, Request, OutputBufferLength, InputBufferLength);
        break;
    ...
    case BMDEV_SYNC_TIMEOUT_API:
        {
            ret = bmdrv_set_sync_timeout(bmdi, Request);
            break;
        }
	...
	case BMDEV_CHECKSUM_MCU:
		{
			...
			if ((BM1684_BOARD_TYPE(bmdi) == BOARD_TYPE_SC5_PRO) ||
				...
				TraceEvents(TRACE_LEVEL_ERROR, TRACE_DEVICE, "bmsophon %d, sc5p not support mcu sheck sum\n", bmdi->dev_index);
				return -1;
			}
