...
int bmdrv_gmem_ioctl_free_mem_u64(struct bm_device_info *bmdi,
                              _In_ WDFREQUEST        Request,
                              _In_ size_t            OutputBufferLength,
                              _In_ size_t            InputBufferLength) {
    int                        ret = 0;
    ...
    NTSTATUS Status = WdfRequestRetrieveInputBuffer(Request, sizeof(bm_device_mem_u64_t), &inDataBuffer, &bufSize);
    if (!NT_SUCCESS(Status)) {
        WdfRequestCompleteWithInformation(Request, Status, 0);
        TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_BGM, "%!FUNC! get input info failed\n");
        return -1;
