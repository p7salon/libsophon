...
static s32 vpp_soft_rst(struct bm_device_info *bmdi, s8 core_id)
{
	...
	/*set dma_stop=1*/
	vpp_reg_write(core_id, bmdi, 0x00080008, BM1684_VPP_CONTROL0);

	/*check dma_stop_active==1*/
	for (count_value = 0; count_value < 1000; count_value++)
	{
		reg_read = vpp_reg_read(core_id, bmdi, BM1684_VPP_STATUS);
		if ((reg_read >> 9) & 0x1)
		{
			active_check++;
			if (active_check == 5)
	...
	/*vpp soft reset*/
	vpp_reg_write(core_id, bmdi, 0x00020000, BM1684_VPP_CONTROL0);

	bm_udelay(10);

	vpp_reg_write(core_id, bmdi, 0x00020002, BM1684_VPP_CONTROL0);


/*deassert dma_stop and check dma_stop_active*/
	vpp_reg_write(core_id, bmdi, 0x00080000, BM1684_VPP_CONTROL0);
...
void vpp_clear_int(struct bm_device_info *bmdi, s32 core_id)
{
	...
	if (core_id == 0)
		vpp0_reg_write(bmdi, BM1684_VPP_INT_CLEAR, 0xffffffff);
	else
		vpp1_reg_write(bmdi, BM1684_VPP_INT_CLEAR, 0xffffffff);
}
...
static s32 vpp_check_hw_idle(s8 core_id, struct bm_device_info *bmdi)
{
	...
	while (1) {
		status = vpp_reg_read(core_id, bmdi, BM1684_VPP_STATUS);
		val = (status>>8) & 0x1;
		if (val) // idle
			break;
	...
	count = 0;
	while (1) {
		status = vpp_reg_read(core_id, bmdi, BM1684_VPP_INT_RAW_STATUS);
		if (status == 0x0)
			break;
		...
		vpp_reg_write(core_id, bmdi, 0xffffffff, BM1684_VPP_INT_CLEAR);
	}
	...
	struct vpp_batch *batch, u64 *vpp_desc_pa)
{
	...
	/*set cmd list addr*/
	vpp_reg_write(core_id, bmdi, (u32)(vpp_desc_pa[0] & 0xffffffff), BM1684_VPP_CMD_BASE);
	vpp_reg_write(core_id, bmdi, (u32)((vpp_desc_pa[0] >> 32) & 0x7), BM1684_VPP_CMD_BASE_EXT);
#if 0
	u32 cmd_base;
	u32 cmd_base_ext;
	...
	vpp_reg_write(core_id, bmdi, 0x00040004, BM1684_VPP_CONTROL0);//cmd list
	vpp_reg_write(core_id, bmdi, 0x03, BM1684_VPP_INT_EN);//interruput mode : vpp_in

	/*start vpp hw work*/
	vpp_reg_write(core_id, bmdi, 0x00010001, BM1684_VPP_CONTROL0);
...
s32 bm1684_vpp_handle_setup(struct bm_device_info *bmdi, struct vpp_batch *batch)
{
...
s32 bm1684_trigger_vpp(struct bm_device_info *bmdi, _In_ WDFREQUEST Request,
				_In_ size_t OutputBufferLength,  _In_ size_t  InputBufferLength)
{
	...
	Status = bm1684_vpp_handle_setup(bmdi, batch);
...
s32 bm1684_vpp_init(struct bm_device_info *bmdi)
{
	s32 i;
