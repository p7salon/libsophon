...
int bm1684_set_chip_index(struct bm_device_info *bmdi)
{
	...
	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"%!FUNC! bm-sophon%d, chip_index = %d,functonnumber=%d\n", bmdi->dev_index, index,bmdi->function_number );
	return index;
}

void bm1684_map_bar(struct bm_device_info *bmdi)
{
	u32  *atu_base_addr;
	u32  *cfg_base_addr;
	u64 bar1_start = 0;
	...
	cfg_base_addr = (u32*)(bmdi->cinfo.bar_info.bar0_vaddr); //0x5f800000
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION,
                TRACE_DEVICE,
                "%!FUNC! bm-sophon%d, function_num = %d\n",
                bmdi->dev_index,
                function_num);
	if (function_num == 0x0) {
	...
	atu_base_addr = (u32*)(bmdi->cinfo.bar_info.bar0_vaddr + REG_OFFSET_PCIE_iATU); //0x5fb00000
	...
	if (bmdi->cinfo.chip_id == 0x1684) {
		...
		REG_WRITE32(atu_base_addr, 0x314, 0x900c000);		//dst addr
	}
	...
	REG_WRITE32(atu_base_addr, 0x2f08, (u32)(bar1_start & 0xffffffff) + BAR1_PART8_OFFSET);		//src addr
	REG_WRITE32(atu_base_addr, 0x2f0C, bar1_start >> 32);
	REG_WRITE32(atu_base_addr, 0x2f10, (u32)(bar1_start & 0xffffffff) + BAR1_PART8_OFFSET + 0xffff); //size 64K
	if (bmdi->cinfo.chip_id == 0x1684) {
		REG_WRITE32(atu_base_addr, 0x2f14, 0x102FF000);            //dst addr
    } else if (bmdi->cinfo.chip_id == 0x1686) {
		REG_WRITE32(atu_base_addr, 0x2f14, 0x12000000);            //dst addr
	}

	REG_WRITE32(atu_base_addr, 0x2f18, 0);
	//ATU9
	// spi  controller 4k
	...
	if (bmdi->cinfo.chip_id == 0x1686) {
		//ATU10
		// SC7-Pro MCU info, 256 Bytes
		REG_WRITE32(atu_base_addr, 0x3100, 0);
		REG_WRITE32(atu_base_addr, 0x3104, 0x80000100);
		//address match mode for bar1, BIT[10:8] are not checked in address match mode
		REG_WRITE32(atu_base_addr, 0x3108, (u32)(bar1_start & 0xffffffff)
		       		+ BAR1_PART9_OFFSET);		//src addr
		REG_WRITE32(atu_base_addr, 0x310C, bar1_start >> 32);
		REG_WRITE32(atu_base_addr, 0x3110, (u32)(bar1_start & 0xffffffff)
			        + BAR1_PART9_OFFSET + 0x500);	//size 512 Bytes
		REG_WRITE32(atu_base_addr, 0x3114, 0x101fb000);		//dst addr
		REG_WRITE32(atu_base_addr, 0x3118, 0);
	}
...
void bm1684_unmap_bar(struct bm_bar_info *bari) {
	u32  *atu_base_addr;
	int i = 0;
    atu_base_addr = (u32*)(bari->bar0_vaddr + REG_OFFSET_PCIE_iATU);
	for (i = 0; i < 30; i++) {
...
static const struct bm_bar_info bm1684x_bar_layout[] = {
{
		.bar0_len = 0x400000,
		.bar0_dev_start = 0x5f800000,
...
static struct bm_bar_info bm_mode_chose_layout_bm1684x[] = {
{
		.bar0_len = 0x400000,
		.bar0_dev_start = 0x5f800000,
...
int bm1684_setup_bar_dev_layout(struct bm_device_info *bmdi, BAR_LAYOUT_TYPE type)
{
	...
	if (type == SETUP_BAR_DEV_LAYOUT) {
		if (bmdi->cinfo.chip_id == 0x1684) {
			...
			bar_layout = bm1684x_bar_layout;
		}
	} else if (type == MODE_CHOSE_LAYOUT) {
		if (bmdi->cinfo.chip_id == 0x1684) {
			bar_layout = bm_mode_chose_layout;
		} else if (bmdi->cinfo.chip_id == 0x1686) {
			bar_layout = bm_mode_chose_layout_bm1684x;
		}
	}
...
void bm1684_pcie_calculate_cdma_max_payload(struct bm_device_info *bmdi)
{
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION,
                TRACE_DEVICE,
                "max_payload = 0x%x, max_rd_req = 0x%x, mode = 0x%x, "
                "total_func_num = 0x%x, max_paload = 0x%x \n",
...
int bmdrv_get_chip_num(struct bm_device_info *bmdi)
{
	...
	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"driver chip num = 0x%x\n", chip_num);
	return chip_num;
}

void bmdrv_init_for_mode_chose(struct bm_device_info *bmdi, struct bm_bar_info *bar_info)
{
	u32  *atu_base_addr;
	u32  *cfg_base_addr;
	...
	atu_base_addr = (u32 *)((u64)bar_info->bar0_vaddr + REG_OFFSET_PCIE_iATU); //0x5fb00000
...
int bmdrv_pcie_get_mode(struct bm_device_info *bmdi)
{
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION,
                TRACE_DEVICE,
                "pcie get mode is 0x%x \n",
                mode);
	return mode;
}

void bmdrv_pcie_set_function1_iatu_config(struct bm_device_info *bmdi)
{
	u32  *atu_base_addr;
	int value = 0x0;
	int function_num = 0x0;
	...
	atu_base_addr = (u32*)(bmdi->cinfo.bar_info.bar2_vaddr); //0x5ff00000
...
void bmdrv_pcie_set_function2_iatu_config(struct bm_device_info *bmdi)
{
	u32  *atu_base_addr;
	int function_num = 0x0;
...
void bmdrv_pcie_set_function3_iatu_config(struct bm_device_info *bmdi)
{
	u32  *atu_base_addr;
	int function_num = 0x0;
...
void bmdrv_set_chip_num(int chip_seqnum, struct bm_device_info *bmdi)
{
	...
	abp_addr = (u32*)(bmdi->cinfo.bar_info.bar0_vaddr + REG_OFFSET_PCIE_iATU + 0x80000); //0x5fb00000
...
void bmdrv_pcie_perst(struct bm_device_info *bmdi)
{
    ...
    TraceEvents(
        TRACE_LEVEL_INFORMATION, TRACE_DEVICE, "%!FUNC! gpio 16 value = 0x%x\n", value);
	value |= (0x1 << 16);
	gpio_reg_write(bmdi, 0x0, value);
	bm_mdelay(300);
}

int bmdrv_pcie_polling_rc_perst(struct bm_bar_info *bari)
{
	int loop = 200;
	int ret = 0;
	void  *atu_base_addr;
	...
	while (!((REG_READ32(atu_base_addr, 0x4c) & (1 << 23)))) {
		if (loop-- > 0) {
			bm_mdelay(1000);
            TraceEvents(
                TRACE_LEVEL_INFORMATION, TRACE_DEVICE, "polling rc perst \n");
		}
		else {
			ret = -1;
            TraceEvents(TRACE_LEVEL_INFORMATION,
                        TRACE_DEVICE,
                        "polling rc perst time ount \n");
			break;
		}
	}
...
int bmdrv_pcie_polling_rc_core_rst(struct bm_bar_info *bari)
{
	int loop = 200;
	int ret = 0;
	void  *atu_base_addr;
...
int bmdrv_pcie_polling_rc_link_state(struct bm_bar_info *bari)
{
	...
	while(value != 0x3) {
		...
		if (count-- > 0) {
            ...
            TraceEvents(TRACE_LEVEL_INFORMATION,
                        TRACE_DEVICE,
                        "wait link state count = %d\n",
                        count);
		} else {
			ret = -1;
            TraceEvents(TRACE_LEVEL_INFORMATION,
                        TRACE_DEVICE,
                        "polling rc link time out\n");
			break;
		}
	}
...
int try_to_link_as_gen1_speed(struct bm_device_info *bmdi, struct bm_bar_info *bari)
{
	int ret = 0;

	bmdrv_pcie_perst(bmdi);
	bmdrv_pcie_polling_rc_perst(bari);
	bmdrv_pcie_polling_rc_core_rst(bari);
	bmdrv_pcie_set_rc_link_speed_gen_x(bari, 0x1);
	bmdrv_pcie_enable_rc(bari);

	if (bmdrv_pcie_polling_rc_link_state(bari) < 0) {
		ret = -1;
		TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"try_to_link_as_gen1_speed still fail \n");
	}

	return ret;
}

int bmdrv_pcie_rc_init(struct bm_device_info *bmdi, struct bm_bar_info *bari)
{
	int count = 0x5;
	int ret = 0;
...
retry:
	...
	if (bmdrv_pcie_polling_rc_link_state(bari) < 0) {
		if (count-- > 0) {
			TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"rc link fail, retry %d\n", count);
			goto retry;
		} else {
			TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"rc link fail, retry still fail\n");
			ret = -1;
		}
	}
	...
	if (ret < 0) {
        TraceEvents(
            TRACE_LEVEL_ERROR, TRACE_DEVICE, "try to link as gen1 speed \n");
		ret = try_to_link_as_gen1_speed(bmdi, bari);
	}
...
int config_iatu_for_function_x(struct bm_device_info *bmdi, struct bm_bar_info *bari)
{
	...
	if (bmdi->cinfo.chip_index > 0)
        ...
        function_num = bmdi->function_number;
	bmdrv_init_for_mode_chose(bmdi, bari);
	io_init(bmdi);

	mode = bmdrv_pcie_get_mode(bmdi);
    TraceEvents(TRACE_LEVEL_ERROR, TRACE_DEVICE, "%!FUNC! mode  = 0x%x\n", mode);
	debug_value = top_reg_read(bmdi,0);
	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"%!FUNC! top value = 0x%x\n",debug_value);
	if (mode == 0x4) {
		TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"%!FUNC! single ep  mode \n");
		return 0;
	}
	...
	if (mode != 0) {
		bmdrv_pcie_rc_init(bmdi, bari);
	}

	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"function_num = 0x%d \n", function_num);
	chip_seqnum = bmdrv_calculate_chip_num(mode, function_num);
	if ((chip_seqnum < 0x1) || (chip_seqnum > 0x4)) {
		TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"chip_seqnum is 0x%x illegal", chip_seqnum);
		return -1;
	}
	...
	case 0x3:
		bmdrv_pcie_set_function3_iatu_config(bmdi);
	case 0x2:
		bmdrv_pcie_set_function2_iatu_config(bmdi);
	case 0x1:
		bmdrv_pcie_set_function1_iatu_config(bmdi);
		ret = bmdrv_pci_bus_scan(bmdi, max_function_num);
		break;
	default:
		ret = 0;
		break;
...
int bmdrv_pci_bus_scan(struct bm_device_info *bmdi, int max_fun_num)
{
	int bus_num = 1;
	int cfg_base_addr = 0;
	int device_num = 0;
	...
	if (bmdi->cinfo.chip_index > 0)
        ...
        root_function_num = bmdi->function_number;
	...
	for (function_num = 0; function_num < max_fun_num; function_num++) {
		...
		while (vendor_id != 0x1e30) {
			...
			bm_mdelay(10);
			if (--count == 0) {
				TraceEvents(TRACE_LEVEL_ERROR, TRACE_DRIVER, "bus num = %d, device = %d, function = %d,   not find, vendor_id = 0x%x \n", bus_num, device_num, function_num, vendor_id);
				break;
			}
		}

		TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"bus num = %d, device = %d, function = %d, vendor_id = 0x%x \n", bus_num, device_num, function_num, vendor_id);
		bm_mdelay(2000);
		if((function_num == 0x0) && ((vendor_id == 0x1e30) || (vendor_id == 0x1f1c))) {
			...
			TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"find 1 bus num = %d,device = %d, function = %d,   find, vendor_id = 0x%x \n", bus_num, device_num, function_num, vendor_id);
			if (max_fun_num == 0x1)
				return 0;
		}
			if((function_num == 0x1) && ((vendor_id == 0x1e30) || (vendor_id == 0x1f1c))) {
				...
				bmdrv_pci_msi_enable(bmdi, cfg_base_addr);
				bmdrv_pci_max_payload_setting(bmdi, cfg_base_addr);
				TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"find 2 bus num = %d,device = %d, function = %d,   find, vendor_id = 0x%x \n", bus_num, device_num, function_num, vendor_id);
				if (max_fun_num == 0x2)
					return 0;
			}
			if((function_num == 0x2) && ((vendor_id == 0x1e30) || (vendor_id == 0x1f1c))) {
...
void pci_slider_bar4_config_device_addr(struct bm_bar_info *bari, u32 addr)
{
	...
	u32 dst_addr = 0;
	u32 temp_addr = 0;
	atu_base_addr = (u32*)(bari->bar0_vaddr + REG_OFFSET_PCIE_iATU); //0x5fb00000
