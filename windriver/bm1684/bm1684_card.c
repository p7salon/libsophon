...
int bm1684_card_get_chip_num(struct bm_device_info *bmdi)
{
	...
	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_CARD, "chip num = %d\n", num);
	return num;
}
...
int bm1684_get_board_version_from_mcu(struct bm_device_info *bmdi)
{
	...
	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_CARD, "flags = %x\n", flags);
	...
	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_CARD, "board_type addr= %p\n", &board_type);
	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_CARD, "hw_version addr= %p\n", &hw_version);
	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_CARD, "mcu_sw_version addr= %p\n", &mcu_sw_version);
	...
	if (board_type == 0x0 && hw_version == 0x4) {
		if (bm_get_board_type(bmdi, (char*)&board_type) != 0) {
			return -1;
		}
	}
...
int bm1684_get_board_type_by_id(struct bm_device_info *bmdi, char *s_board_type, int id)
{
	...
	case BOARD_TYPE_SC7_PRO:
		strncpy(s_board_type, "224T", 10);
		break;
	case BOARD_TYPE_SC7_FP150:
		strncpy(s_board_type, "FP150", 10);
		break;
...
int bm1684_get_board_version_by_id(struct bm_device_info *bmdi, char *s_board_version, int board_id, int pcb_version, int bom_version)
{
    UNREFERENCED_PARAMETER(bmdi);
	int ret = 0;
	...
	case BOARD_TYPE_SC7_PRO:
		if (board_version == 0x11)
			...
			_snprintf(s_board_version, 10, "V1_%d", board_version);
		break;
	case BOARD_TYPE_SC7_FP150:
...
{
	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"bm1684 tpu reset\n");
...
{
	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE,"bm1684 gdma reset\n");
