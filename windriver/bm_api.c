...
int bmdrv_api_init(struct bm_device_info *bmdi, u32 channel)
{
	...
	if (BM_MSGFIFO_CHANNEL_XPU == channel){
		...
		bmdi->lib_dyn_info = (struct bmcpu_lib *)MmAllocateNonCachedMemory(sizeof(struct bmcpu_lib));
		...
		bmdi->lib_info = (struct bmcpu_lib *)MmAllocateNonCachedMemory(sizeof(struct bmcpu_lib));
...
void bmdrv_api_deinit(struct bm_device_info *bmdi, u32 channel)
{
	...
	if (BM_MSGFIFO_CHANNEL_XPU == channel){
		...
		list_for_each_entry_safe(lib_temp, lib_next, &lib_dyn_info->lib_list, lib_list, struct bmcpu_lib) {
			...
			MmFreeNonCachedMemory((struct bmcpu_lib *) lib_temp,sizeof(struct bmcpu_lib));
			//API_P_FREE(lib_temp);
		}
		ExReleaseFastMutex(&lib_dyn_info->bmcpu_lib_mutex);
		//API_P_FREE(bmdi->lib_dyn_info);
        MmFreeNonCachedMemory((struct bmcpu_lib*)bmdi->lib_dyn_info, sizeof(struct bmcpu_lib));
	} else {
        ...
        MmFreeNonCachedMemory((struct bmcpu_lib*)bmdi->lib_info, sizeof(struct bmcpu_lib));
	}
...
int bmdrv_api_dyn_get_func_process(struct bm_device_info *bmdi, bm_api_ext_t *p_bm_api)
{
	...
	func_node = (struct bmdrv_exec_func *)MmAllocateNonCachedMemory(sizeof(struct bmdrv_exec_func));
	if (!func_node){
    ...
    if(fun_flag == 1) {
        ...
        MmFreeNonCachedMemory((struct bmdrv_exec_func*)func_node, sizeof(struct bmdrv_exec_func));
...
int bmdrv_api_dyn_load_lib_process(struct bm_device_info *bmdi, bm_api_ext_t *p_bm_api)
{
    ...
    list_for_each_entry_safe(lib_temp, lib_next, &lib_info->lib_list, lib_list, struct bmcpu_lib){
    	if (!memcmp(lib_temp->md5, api_cpu_load_library_internal.md5, MD5SUM_LEN)) {
			...
			//lib_node = (struct bmcpu_lib*)API_P_ALLOC(sizeof(struct bmcpu_lib));
			lib_node = (struct bmcpu_lib *)MmAllocateNonCachedMemory(sizeof(struct bmcpu_lib));
			InitializeListHead(&(lib_node->lib_list));
    		strncpy(lib_node->lib_name, (const char *)api_cpu_load_library_internal.lib_name, LIB_MAX_NAME_LEN);
    ...
    ExAcquireFastMutex(&lib_info->bmcpu_lib_mutex);
    list_for_each_entry_safe(lib_temp, lib_next, &lib_info->lib_list, lib_list, struct bmcpu_lib) {
    	if (!strncmp(lib_temp->lib_name, (const char*)api_cpu_load_library_internal.lib_name, LIB_MAX_NAME_LEN)){
    ...
    if (del_lib == 1) {
    	list_for_each_safe(pos, pos_next, &(bmdi->exec_func.func_list)) {
    		if (!memcmp(((struct bmdrv_exec_func *)pos)->exec_func.md5, api_cpu_load_library_internal.md5, MD5SUM_LEN)) {
    			...
    			MmFreeNonCachedMemory((struct bmdrv_exec_func *)pos,sizeof(struct bmdrv_exec_func));
				//API_P_FREE((struct bmdrv_exec_func*)pos);
    		}
    	}
                     ...
                     _In_ size_t            InputBufferLength)
{
    ...
    RtlCopyMemory(&loaded_lib, InDataBuffer, inbufSize);
    loaded_lib.loaded = 0;
    ...
    ExReleaseFastMutex(&lib_info->bmcpu_lib_mutex);
    RtlCopyMemory(OutDataBuffer, &loaded_lib, outbufSize);
    WdfRequestCompleteWithInformation(Request, STATUS_SUCCESS, outbufSize);
    return 0;
}

int bmdrv_set_sync_timeout(struct bm_device_info *bmdi,
                          _In_ WDFREQUEST        Request)
{
	size_t bufSize;
	PVOID InDataBuffer;
	...
	Status = WdfRequestRetrieveInputBuffer(Request, sizeof(int), &InDataBuffer, &bufSize);
	if (!NT_SUCCESS(Status)) {
		TraceEvents(TRACE_LEVEL_ERROR,
					TRACE_API,
					"bm-sophon%d copy_from_user fail, status is %d\n",
					bmdi->dev_index, Status);
		ret = -1;
		goto End;
	}
	RtlCopyMemory(&timeout, (int *)InDataBuffer, sizeof(int));
	if (timeout < 0) {
		Status = STATUS_INVALID_PARAMETER;
		TraceEvents(TRACE_LEVEL_ERROR,
					TRACE_API,
					"set sync timeout error!\n");
		ret = -1;
		goto End;
	}
